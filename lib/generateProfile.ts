...
/**
 * Genereert een nieuw voorkeursprofiel op basis van de feedback van de laatste
 * 90 dagen (recente feedback telt dubbel) en slaat het op in user_profile.
 * Wordt aangeroepen door zowel de cron-route als de on-demand profile-update.
 */
export async function generateProfile(): Promise<GenerateProfileResult> {
  ...
  if (!feedback || feedback.length === 0) {
    return { ok: false, skipped: true, reason: 'No feedback yet' }
  }
...
Schrijf alleen het profiel, geen inleiding of afsluiting.`
  ...
  try {
    ...
    if (!profile) return { ok: false, error: 'No profile generated' }
    ...
    return { ok: true, length: profile.length }
  } catch (e) {
